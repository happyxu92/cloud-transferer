...
def cmd_task_progress(
    ...
    async def _main() -> None:
        ...
        if not rows:
            console.print(f"[yellow]job={job or 'ALL'} 当前没有处于 copying 的任务[/]")
            return
        ...
        try:
            ...
            console.print(f"[red]读取 AList 后台任务失败: {e}[/]")
            return
        finally:
            await client.aclose()

        table = Table(title=f"Task Progress (job={job or 'ALL'})")
        for col in ["ID", "Job", "Progress", "AList", "Size(MB)", "Src", "Dst", "Error"]:
            table.add_column(col)

        for t in rows:
            backend = task_map.get(t["alist_task_id"])
            if not t["alist_task_id"]:
                progress = "starting"
